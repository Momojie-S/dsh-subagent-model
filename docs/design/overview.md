...
2. **上下文继承**：`fresh_context` 参数。**省略时默认继承当前对话**——子代理以调用那一刻父会话的**最新完成轮次**为种子（fork 语义）；传 `true` 则干净上下文（spawn 语义）。
...
2. **上下文继承（同样零新传输）**：`ctx.subagents` 是按名字选 provider 的注册表，`start(name, request)` 本来就接受每次调用指定 provider。插件在 `execute()` 里按 `fresh_context`（缺省回退 `defaultContext`，默认 `inherit`）选 provider 名：继承 → `inheritProvider`（默认 `fork`），fresh → `provider`（如 `spawn`）。
...
4. **继承模式恒 one-shot**（对齐上游 `subagent_fork`，ADR-0003）：continuable 子代理的 `report` 通道会排在继承历史之前，破坏前缀复用；且 continuable fork 的前缀在创建时冻结一次，后续轮次不会跟进父会话新内容——与"取最新"的目标相反。fresh 模式的后台策略不受影响（`backgroundMode` 只作用于 fresh）。
...
- **成本提示**（写进了工具描述）：继承模式下整段对话会在子代理的路由上重新计费/预填充；换模型继承（本插件的典型用法）拿不到 provider 侧前缀复用。自包含任务应传 `fresh_context: true`。
